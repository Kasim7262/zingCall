  ...
  return (
    <section className="relative h-screen w-full overflow-hidden pt-4 text-white">
      <div className="relative flex size-full items-center justify-center">
        ...
        >
          {call && <ChatComponent callId={call.id} />}
        </div>

        {/* Live Captions Sidebar */}
        <div
        //   className={cn('h-[calc(100vh-86px)] hidden ml-2', {
        //     'show-block': showCaptions,
        //   })}
        >
          {startCaptions && <LiveCaptions startCaptions={startCaptions} />}
        </div>
      ...
      <div className="fixed bottom-0 flex w-full items-center justify-center gap-5">
        ...
        {/* Toggle Live Captions Button */}
        <button onClick={() => setStartCaptions((prev) => !prev)}>
          <div className="cursor-pointer rounded-2xl bg-[#19232d] px-4 py-2 hover:bg-[#4c535b]">
            <span className="text-white">
              {startCaptions ? 'Stop Captions' : 'Start Captions'}
            </span>
          </div>
        </button>
